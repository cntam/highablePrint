...
for($spage = 0;$spage< $stotal;$spage++){
    ...
    $spreadsheet->getActiveSheet()->setCellValue('C1', $spage);

    if($spage > 0){
        $prnum = $maxlist <= (4 + 5 * $spage) ? $maxlist : (4 + 5 * $spage);
...
$spreadsheet->getActiveSheet()->setCellValue('A14', '特殊工序：');
    ...
    $maxitem = 0;    //第一个表格中 item 行数最大的数值是
    for($q = 0; $q<= $nowprnum ;$q++){
        $itemarr = array();
        $itemarr = json_decode(stripcslashes($cpsp2[$q][0]["item"]), true);
